...
int mail_criteria_string(STRINGLIST **s, char **r)
{
    ...
    case '"': /* quoted string */
        if (!strchr(c, '\\'))
        { /* simple case, no escape chars inside */
            if (strchr(c + 1, '"'))
                end = "\"";
            else
        ...
        { /* we need to strip escape char cos it will be passed as literal */
            ...
            if (fflag != 2)
                return NIL; /* no final quote or string is too long for buffer */
            break;
            // new code
        }
