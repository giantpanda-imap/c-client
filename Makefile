...
#	 (see lnp, and slx)
# lnxs	Linux with traditional passwords and crypt() in the C library
#	 (see lnp, and slx)  as Shared Lib
# lnp	Linux with Pluggable Authentication Modules (PAM)
# lnps	Linux with Pluggable Authentication Modules (PAM) as Shared Lib
# lmd	Mandrake Linux
# lsu	SuSE Linux
...
# slxs	Linux using -lcrypt to get the crypt() function as Shared Lib
...
bsi lnp lnps lyn mct mnt slx slxs: an
	$(BUILD) BUILDTYPE=$@
...
an:
	...
	$(TOOLS)/$@ "$(LN)" src c-client
	$(TOOLS)/$@ "$(LN)" src/ansilib c-client
	$(TOOLS)/$@ "$(LN)" src/charset c-client
	$(TOOLS)/$@ "$(LN)" src/$(SYSTEM) c-client
	$(LN) $(TOOLS)/$@ .
...
clean:
	...
	$(SH) -c '$(RM) an OSTYPE SPECIALS c-client ip6 || true'
	$(CD) tools;$(MAKE) clean

install: all
	install -D c-client/libc-client.so* $(DESTDIR)/usr/lib/
	install -D c-client/c-client.a $(DESTDIR)/usr/lib/libc-client.a
